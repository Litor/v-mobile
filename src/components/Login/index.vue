          ...
          <form class="signin-form form-horizontal" @submit.prevent="login($signinValidation)" novalidate>
            <div class="form-group">
              <div class="input-group">
                ...
                <input type="text" v-model="user.email" v-validate:email="{ required: true, minlength: 3, maxlength: 30, email:true }" class="form-control" :class="[$signinValidation.email.invalid ? 'ng-invalid' : 'ng-valid', $signinValidation.email.dirty ? 'ng-dirty' : '']" placeholder="邮箱" />
              </div>
            </div>
            <div class="form-group">
              <div class="input-group">
                <div class="input-group-addon"><i class="fa fa-unlock-alt"></i></div>
                <input type="password" v-model="user.password" v-validate:password="{ required: true }" class="form-control" :class="[$signinValidation.password.invalid ? 'ng-invalid' : 'ng-valid', $signinValidation.password.dirty ? 'ng-dirty' : '']" placeholder="密码" />
              </div>
            </div>
            <div class="form-group">
              <div class="col-xs-6 captcha-code">
                <input class="form-control" v-model="user.captcha" v-validate:captcha="{ required: true, maxlength: 6 }" :class="[$signinValidation.captcha.invalid ? 'ng-invalid' : 'ng-valid', $signinValidation.captcha.dirty ? 'ng-dirty' : '']" type="text" maxlength="6" placeholder="验证码" />
              </div>
